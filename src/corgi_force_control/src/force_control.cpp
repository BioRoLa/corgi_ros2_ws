...
Eigen::MatrixXd calculate_P_poly(int rim, double alpha){
    ...
    if      (rim == 1) P_poly = rot_alpha * (H_l_poly-U_l_poly) / legmodel.R * legmodel.radius + U_l_poly;
    else if (rim == 2) P_poly = rot_alpha * (F_l_poly-L_l_poly) / legmodel.R * legmodel.radius + L_l_poly;
    else if (rim == 3) P_poly = G_poly / legmodel.R * legmodel.radius;
    else if (rim == 4) P_poly = rot_alpha * (G_poly-L_r_poly)   / legmodel.R * legmodel.radius + L_r_poly;
    else if (rim == 5) P_poly = rot_alpha * (F_r_poly-U_r_poly) / legmodel.R * legmodel.radius + U_r_poly;
...
Eigen::MatrixXd calculate_torque(Eigen::MatrixXd eta, Eigen::MatrixXd force){
    ...
    Eigen::MatrixXd torque = jacobian.transpose() * force;
...
int main(int argc, char **argv) {
    ...
    std::vector<Eigen::MatrixXd> force_cmd_modules = {
    ...
    std::vector<Eigen::MatrixXd> torque_cmd_modules = {
        Eigen::MatrixXd::Zero(2, 1),
        Eigen::MatrixXd::Zero(2, 1),
        Eigen::MatrixXd::Zero(2, 1),
        Eigen::MatrixXd::Zero(2, 1)
    ...
    std::vector<Eigen::MatrixXd> eta_cmd_modules = {
        Eigen::MatrixXd::Zero(2, 1),
        Eigen::MatrixXd::Zero(2, 1),
        Eigen::MatrixXd::Zero(2, 1),
        Eigen::MatrixXd::Zero(2, 1)
    ...
    while (ros::ok()) {
        ...
        for (int i=0; i<4; i++){
            eta_cmd_modules[i] << imp_cmd_modules[i]->theta, imp_cmd_modules[i]->beta;
            force_cmd_modules[i] << imp_cmd_modules[i]->Fx, imp_cmd_modules[i]->Fy;
