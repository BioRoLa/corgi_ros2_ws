...
void ImpedanceCmdPublisherNode::execute_initialization_phase() {
    ...
    for (int i=0; i<2000; i++){
        ...
        eta = legmodel.move(imp_cmd_modules_[1]->theta, imp_cmd_modules_[1]->beta, {0.0, h_/2000.0});
...
void ImpedanceCmdPublisherNode::execute_control_phase() {
    ...
    while (rclcpp::ok()) {
        if (loop_count < 2000) {
            ...
            if (loop_count < 2200) { dh += h_change_height_/1800.0/200.0; }
            else if (loop_count < 3800) { dh =   h_change_height_/1800.0; }
            else if (loop_count < 4000) { dh -=  h_change_height_/1800.0/200.0; }
            else if (loop_count < 4200) { dh -=  h_change_height_/1800.0/200.0; }
            else if (loop_count < 5800) { dh =  -h_change_height_/1800.0; }
            else if (loop_count < 6000) { dh +=  h_change_height_/1800.0/200.0; }
            else if (loop_count < 6200) { dh +=  h_change_height_/1800.0/200.0; }
            else if (loop_count < 7800) { dh =   h_change_height_/1800.0; }
            else if (loop_count < 8000) { dh -=  h_change_height_/1800.0/200.0; }
            else if (loop_count < 8200) { dh -=  h_change_height_/1800.0/200.0; }
            else if (loop_count < 9800) { dh =  -h_change_height_/1800.0; }
            else if (loop_count < 10000) { dh += h_change_height_/1800.0/200.0; }

            eta = legmodel.move(imp_cmd_modules_[1]->theta, imp_cmd_modules_[1]->beta, {0.0, dh});
