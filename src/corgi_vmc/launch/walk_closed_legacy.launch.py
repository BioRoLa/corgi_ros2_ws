#!/usr/bin/env python3
"""Real-robot VMC launch — odom_legacy mode.
...
-----------
  ...
  corgi_force_control         sub /force/state, /impedance/command
                              pub /motor/command
  corgi_vmc (walk_closed_time or walk_closed_dist)
...
def generate_launch_description():
    package_share_dir = get_package_share_directory('corgi_vmc')
    bag_script = os.path.join(package_share_dir, 'script', 'vmc_closed_legacy_bag.sh')
