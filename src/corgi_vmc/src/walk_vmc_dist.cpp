...
constexpr const char* kVariantName = "walk_vmc_dist";
...
int main(int argc, char **argv) {
    ...
    const double decel_dist =
        0.5 * walk_cfg.cruise_velocity * (walk_cfg.ramp_loops * vmc.dt) * walk_cfg.decel_margin;

    RCLCPP_INFO(node->get_logger(), "Contact source: %s", walk_cfg.contact_source.c_str());
    RCLCPP_INFO(node->get_logger(),
                "Gait: stand_height=%.2f cruise_vel=%.2f step_length=%.2f step_height=%.3f ramp_loops=%d stop_x=%.2f decel_dist=%.3f",
                walk_cfg.stand_height, walk_cfg.cruise_velocity, walk_cfg.step_length,
                walk_cfg.step_height, walk_cfg.ramp_loops, walk_cfg.stop_x, decel_dist);
    ...
    while (rclcpp::ok()) {
        ...
        if (trigger) {
            ...
            const int settling_timeout = static_cast<int>(0.4 * vmc.freq);
            ...
            while (rclcpp::ok()) {
                ...
                if (phase == WalkPhase::RAMP_UP) {
                    ...
                    if (ramp_count >= walk_cfg.ramp_loops) {
                        vmc.target_vel_x = walk_cfg.cruise_velocity;
                        phase = WalkPhase::CRUISE;
                    }
                    walk_gait.set_velocity(vmc.target_vel_x);
                } else if (phase == WalkPhase::CRUISE) {
                    if (vmc.robot_pos[0] >= walk_cfg.stop_x - decel_dist) {
                        RCLCPP_INFO(node->get_logger(),
                                    "CRUISE -> DECEL: pos_x=%.3f decel trigger at %.3f",
                                    vmc.robot_pos[0], walk_cfg.stop_x - decel_dist);
                        phase = WalkPhase::DECEL;
                    }
                } else if (phase == WalkPhase::DECEL) {
                    ++decel_count;
                    vmc.target_vel_x -= walk_cfg.cruise_velocity / walk_cfg.ramp_loops;
                    if (decel_count >= walk_cfg.ramp_loops) {
                ...
                std::cout << "Phase: "
                          ...
                          << " stop_x=" << walk_cfg.stop_x
                          << " decel_at=" << walk_cfg.stop_x - decel_dist << std::endl;
