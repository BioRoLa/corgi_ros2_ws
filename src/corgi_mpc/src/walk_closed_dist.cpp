// walk_closed_dist.cpp
// Distance-driven closed-loop walk controller.
// The robot uses a WalkPhase state machine (RAMP_UP → CRUISE → DECEL → SETTLING)
// and stops near a target X-axis position (stop_x) rather than after a fixed time.
//
// Stopping logic:
//   • DECEL is triggered when robot_pos[0] >= stop_x - decel_dist.
//   • decel_dist = 0.5 × cruise_velocity × (ramp_loops × dt) × decel_margin
//   • SETTLING waits until all four legs are in stance, with a timeout guard.
//
// NOTE: X/Y position weights in the MPC Q matrix are 0, so target_pos_x does NOT
// drive any force output. Forward motion is controlled purely through target_vel_x.
// target_pos_x is accumulated here (same logic as walk_closed_time) solely for
// logging and Z-axis / velocity reference channel consistency.
...
int main(int argc, char **argv) {
    ...
    RCLCPP_INFO(node->get_logger(), "Corgi MPC Starts (walk_closed_dist)");
    ...
    // ── Distance-driven stop parameters ───────────────────────────────────
    // stop_x:      target stop position on the X axis in odom/world frame (m).
    // decel_margin: values > 1.0 start braking earlier than the minimum needed.
    const double stop_x       = gait_read_double("stop_x");
    const double decel_margin = gait_read_double("decel_margin");
    ...
    // mpc.target_loop is unused in walk_closed_dist (distance-driven).

    // Decel trigger distance:
    //   During ramp-down (ramp_loops cycles), average velocity = cruise_velocity / 2.
    //   Theoretical stop distance = (cruise_velocity / 2) × (ramp_loops × dt).
    //   We multiply by decel_margin so braking starts earlier (decel_margin > 1.0).
    const double decel_dist = 0.5 * cruise_velocity * (ramp_loops * mpc.dt) * decel_margin;

    RCLCPP_INFO(node->get_logger(),
        "Gait: stand_height=%.2f  cruise_vel=%.2f  step_length=%.2f  "
        "step_height=%.3f  ramp_loops=%d  stop_x=%.2f  decel_dist=%.3f m (margin=%.1f)",
        stand_height, cruise_velocity, step_length,
        step_height_cfg, ramp_loops, stop_x, decel_dist, decel_margin);
    ...
    while (rclcpp::ok()) {
        ...
        if (trigger){
            ...
            // ── Distance-driven walk phase state machine ───────────────────
            enum class WalkPhase { RAMP_UP, CRUISE, DECEL, SETTLING };
            WalkPhase phase   = WalkPhase::RAMP_UP;
            int ramp_count    = 0;
            int decel_count   = 0;
            int settling_count = 0;
            // Settling timeout ≈ 2 × swing_time × freq (= 2 × 0.2 s × 100 Hz = 40 cycles).
            // Guarantees the loop always terminates even if a leg is stuck in swing.
            const int settling_timeout = 40;

            while (rclcpp::ok()) {
                rclcpp::spin_some(node);

                // ── Distance-driven velocity profile ──────────────────────
                if (phase == WalkPhase::RAMP_UP) {
                    ++ramp_count;
                    mpc.target_vel_x += velocity / ramp_loops;
                    if (ramp_count >= ramp_loops) {
                        mpc.target_vel_x = velocity; // clamp to exact cruise value
                        phase = WalkPhase::CRUISE;
                    }
                    walk_gait.set_velocity(mpc.target_vel_x);
                }
                else if (phase == WalkPhase::CRUISE) {
                    // Trigger decel when the estimated position is within decel_dist of stop_x.
                    if (mpc.robot_pos[0] >= stop_x - decel_dist) {
                        RCLCPP_INFO(node->get_logger(),
                            "CRUISE → DECEL: pos_x=%.3f  decel trigger at %.3f",
                            mpc.robot_pos[0], stop_x - decel_dist);
                        phase = WalkPhase::DECEL;
                    }
                }
                else if (phase == WalkPhase::DECEL) {
                    ++decel_count;
                    mpc.target_vel_x -= velocity / ramp_loops;
                    if (decel_count >= ramp_loops) {
                        mpc.target_vel_x = 0.0; // clamp to exact zero
                        walk_gait.set_velocity(0.0);
                        phase = WalkPhase::SETTLING;
                        RCLCPP_INFO(node->get_logger(), "DECEL → SETTLING");
                    } else {
                        walk_gait.set_velocity(mpc.target_vel_x);
                    }
                }
                else { // SETTLING
                    // Wait until all four legs are in stance before halting.
                    const auto sp = walk_gait.get_swing_phase();
                    const bool all_stance = (sp[0] == 0 && sp[1] == 0 && sp[2] == 0 && sp[3] == 0);
                    if (all_stance || settling_count >= settling_timeout) {
                        if (!all_stance) {
                            RCLCPP_WARN(node->get_logger(),
                                "SETTLING timeout (%d cycles): not all legs in stance, stopping anyway",
                                settling_timeout);
                        }
                ...
                // logging and for Z-axis / velocity reference channel consistency.
                ...
                std::cout << "Phase: " <<
                    (phase == WalkPhase::RAMP_UP  ? "RAMP_UP"  :
                     phase == WalkPhase::CRUISE   ? "CRUISE"   :
                     phase == WalkPhase::DECEL    ? "DECEL"    : "SETTLING")
                    << "  stop_x=" << stop_x << "  decel_at=" << stop_x - decel_dist << std::endl;
