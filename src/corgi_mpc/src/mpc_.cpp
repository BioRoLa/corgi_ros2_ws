...
void ModelPredictiveController::init_matrices(const double *ra, const double *rb, const double *rc, const double *rd) {
    ...
    // 1) compute the *actual* inertia
    double Ixx = m*(l*l + h*h)/12.0;
    double Iyy = m*(w*w + h*h)/12.0;
    double Izz = m*(w*w + l*l)/12.0;

    // 2) build body‐frame inertia and invert once
    Eigen::Matrix3d BI_body;
    BI_body << Ixx, 0,   0,
               0,   Iyy, 0,
               0,   0,   Izz;
    Eigen::Matrix3d Iinv = BI_body.inverse();  // now truly I^{-1}

    // 3) fill Bc with I^{-1} [r]_x
    Bc.setZero();
    ...
    Q.diagonal() << 100,  500,  0,    // roll, pitch, yaw
                    10,   0,    300,  // x, y, z
                    10,   1,    0,    // ω_x, ω_y, ω_z
                    1,    0,    50,  // v_x, v_y, v_z
                    0;            // additional state
...
Eigen::VectorXd ModelPredictiveController::step(const Eigen::VectorXd &x, const Eigen::VectorXd &x_ref,
    ...
    // === Constraints ===
    const int constraint_per_step = (3 + 3 + 2 + 1) * 4;
    ...
    for (int k = 0; k < (N - 1); ++k) {
        for (int foot = 0; foot < 4; ++foot) {
            ...
            // 3. Friction cone
            // constraints.insert(row, base_idx) =  1.0;
            // constraints.insert(row, base_idx + 2) = -friction_coef;
            // lower_bound(row) = -1e20; upper_bound(row) = 0.0; ++row;

            // constraints.insert(row, base_idx) = -1.0;
            // constraints.insert(row, base_idx + 2) = -friction_coef;
            // lower_bound(row) = -1e20; upper_bound(row) = 0.0; ++row;

            constraints.insert(row, base_idx) = 0;
            constraints.insert(row, base_idx + 2) = 0;
            lower_bound(row) = -1; upper_bound(row) = 1; ++row;
