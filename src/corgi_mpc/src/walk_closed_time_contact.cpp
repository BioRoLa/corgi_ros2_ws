// walk_closed_time_contact.cpp
// Time-driven closed-loop walk controller using WalkGaitWithContact.
...
int main(int argc, char **argv) {
    ...
    auto node = rclcpp::Node::make_shared("corgi_mpc_contact");

    node->declare_parameter<std::string>("config_profile", "sim");
    node->declare_parameter<std::string>("state_source", "odom_legacy");
    node->declare_parameter<std::vector<double>>("ground_offset", {0.0, 0.0, 0.0, 0.0});
    node->declare_parameter<double>("probe_speed", 0.05);
    ...
    RCLCPP_INFO(node->get_logger(), "Corgi MPC Starts (walk_closed_time_contact)");
    ...
    if (contact_source != "gmo") {
        RCLCPP_WARN(node->get_logger(),
                    "walk_closed_time_contact is most useful with contact_source=gmo; "
                    "current contact_source=gait uses the nominal gait phase as fallback");
    }
    ...
    WalkGaitWithContact walk_gait(sim, 0, mpc.freq);
    ...
    walk_gait.set_step_height(step_height_cfg);

    const std::vector<double> ground_offset_param =
        node->get_parameter("ground_offset").as_double_array();
    if (ground_offset_param.size() == 4) {
        walk_gait.set_ground_offset({
            ground_offset_param[0],
            ground_offset_param[1],
            ground_offset_param[2],
            ground_offset_param[3]});
    } else {
        RCLCPP_WARN(node->get_logger(),
                    "Parameter ground_offset must have exactly 4 values; using zeros");
    }

    walk_gait.set_probe_speed(node->get_parameter("probe_speed").as_double());
    walk_gait.set_contact_filter(
        node->get_parameter("enable_contact_filter").as_bool(),
        node->get_parameter("contact_filter_swing_accept_ratio").as_double(),
        node->get_parameter("contact_filter_on_count").as_int(),
        node->get_parameter("contact_filter_off_count").as_int());
    RCLCPP_INFO(node->get_logger(),
                "WalkGaitWithContact: probe_speed=%.3f, contact_filter=%s",
                walk_gait.probe_speed,
                node->get_parameter("enable_contact_filter").as_bool() ? "enabled" : "disabled");

    walk_gait.initialize(init_eta, step_length);
    walk_gait.set_velocity(mpc.target_vel_x);
    std::array<int, 4> previous_swing_phase = walk_gait.get_swing_phase();
    ...
    while (rclcpp::ok()) {
        ...
        if (trigger){
            ...
            while (rclcpp::ok()) {
                ...
                std::array<bool, 4> contact_input = {
                    previous_swing_phase[0] == 0,
                    previous_swing_phase[1] == 0,
                    previous_swing_phase[2] == 0,
                    previous_swing_phase[3] == 0
                };
                ...
                walk_gait.set_contact_state(contact_input);

                // get next eta
                mpc.eta_list = walk_gait.step();
                const auto gait_debug = walk_gait.get_debug_state();
                const auto swing_phase = gait_debug.swing_phase;
                previous_swing_phase = swing_phase;
                ...
                for (int i = 0; i < 4; i++) {
                    const bool stance_contact =
                        swing_phase[i] == 0 && gait_debug.contact_state[i] && !gait_debug.late_probing[i];
                    const bool early_swing_contact =
                        swing_phase[i] != 0 && gait_debug.early_contact[i] && gait_debug.contact_state[i];
                    const bool support_contact = stance_contact || early_swing_contact;

                    if (!support_contact && touched[i]) {
                        selection_matrix[i] = false;
                        touched[i] = false;
                        imp_cmd_modules[i]->by = mpc.By_swing;
                        imp_cmd_modules[i]->ky = mpc.Ky_swing;
                    }
                    else if (support_contact && !touched[i]) {
                        selection_matrix[i] = true;
                        touched[i] = true;
                        imp_cmd_modules[i]->by = mpc.By_stance;
                        imp_cmd_modules[i]->ky = mpc.Ky_stance;
                    }
                    swing_phase_msg.data[i] = support_contact ? 0 : 1;
                }
