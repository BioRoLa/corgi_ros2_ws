...
namespace corgi
{
    ...
    inline Params load_params(const std::string &yaml_path)
    {
        ...
        if (auto n = root["contact"])
        {
            ...
            p.contact_gamma_threshold_high = detail::val(n, "gamma_threshold_high", p.contact_gamma_threshold_high);
            p.contact_gamma_threshold_low = detail::val(n, "gamma_threshold_low", p.contact_gamma_threshold_low);
        }
