...
namespace corgi
{
    ...
    {
        RawRecord r;
        r.sim_pos_x = d.sim_pos_x;
        r.sim_pos_y = d.sim_pos_y;
        r.sim_pos_z = d.sim_pos_z;
        r.sim_orien_x = d.sim_orien_x;
        r.sim_orien_y = d.sim_orien_y;
        r.sim_orien_z = d.sim_orien_z;
        r.sim_orien_w = d.sim_orien_w;
        r.imu_seq = d.imu_seq;
        r.imu_sec = d.imu_sec;
        r.imu_nsec = d.imu_nsec;
        r.imu_orien_x = d.imu_orien_x;
        r.imu_orien_y = d.imu_orien_y;
        r.imu_orien_z = d.imu_orien_z;
        r.imu_orien_w = d.imu_orien_w;
        r.imu_ang_vel_x = d.imu_ang_vel_x;
        r.imu_ang_vel_y = d.imu_ang_vel_y;
        r.imu_ang_vel_z = d.imu_ang_vel_z;
        r.imu_lin_acc_x = d.imu_lin_acc_x;
        r.imu_lin_acc_y = d.imu_lin_acc_y;
        r.imu_lin_acc_z = d.imu_lin_acc_z;
        r.state_theta_a = d.state_theta_a;
        r.state_beta_a = d.state_beta_a;
        r.state_vel_r_a = d.state_vel_r_a;
        r.state_vel_l_a = d.state_vel_l_a;
        r.state_trq_r_a = d.state_trq_r_a;
        r.state_trq_l_a = d.state_trq_l_a;
        r.state_theta_b = d.state_theta_b;
        r.state_beta_b = d.state_beta_b;
        r.state_vel_r_b = d.state_vel_r_b;
        r.state_vel_l_b = d.state_vel_l_b;
        r.state_trq_r_b = d.state_trq_r_b;
        r.state_trq_l_b = d.state_trq_l_b;
        r.state_theta_c = d.state_theta_c;
        r.state_beta_c = d.state_beta_c;
        r.state_vel_r_c = d.state_vel_r_c;
        r.state_vel_l_c = d.state_vel_l_c;
        r.state_trq_r_c = d.state_trq_r_c;
        r.state_trq_l_c = d.state_trq_l_c;
        r.state_theta_d = d.state_theta_d;
        r.state_beta_d = d.state_beta_d;
        r.state_vel_r_d = d.state_vel_r_d;
        r.state_vel_l_d = d.state_vel_l_d;
        r.state_trq_r_d = d.state_trq_r_d;
        r.state_trq_l_d = d.state_trq_l_d;

        r.state_gamma_a = d.state_gamma_a;
        r.state_vel_h_a = d.state_vel_h_a;
        r.state_trq_h_a = d.state_trq_h_a;
        r.state_gamma_b = d.state_gamma_b;
        r.state_vel_h_b = d.state_vel_h_b;
        r.state_trq_h_b = d.state_trq_h_b;
        r.state_gamma_c = d.state_gamma_c;
        r.state_vel_h_c = d.state_vel_h_c;
        r.state_trq_h_c = d.state_trq_h_c;
        r.state_gamma_d = d.state_gamma_d;
        r.state_vel_h_d = d.state_vel_h_d;
        r.state_trq_h_d = d.state_trq_h_d;
        return r;
    }
    ...
    int OfflineTestNode::run()
    {
        ...
        // ── Load CSV ────────────────────────────────────────────────
        const auto csv_file_path = (std::filesystem::path(__FILE__).parent_path().parent_path().parent_path() / "data" / (params_.csv_filename + ".csv")).string();
        if (!quiet)
        ...
        if (params_.simulate_imu_noise)
        {
            ...
            if (!quiet)
                std::cout << "IMU noise simulation ENABLED (seed=" << params_.imu_noise_seed
                          << ", fs=" << (1.0 / dt) << " Hz)\n";
        }
        ...
        if (!quiet)
            std::cout << "Noise params: sigma_a=[" << params_.sigma_a.transpose()
                      << "], sigma_leg_vec=[" << params_.sigma_leg_vec.transpose() << "]\n";
        if (!quiet)
            std::cout << "GMO input:   " << (params_.use_esekf_state ? "ESEKF estimated state" : "Ground truth (sim_pos)") << "\n";
        ...
        for (size_t i = start_index; i < data.size() && processed_count < max_processed; ++i)
        {
            ...
            // ── Log CSV row ─────────────────────────────────────────
            {
                ...
                for (int j = 0; j < 4; ++j)
                    esekf_out << diag[j].d_squared << ",";
                for (int j = 0; j < 4; ++j)
                    esekf_out << (diag[j].rejected ? 1 : 0) << ",";
                for (int j = 0; j < 4; ++j)
