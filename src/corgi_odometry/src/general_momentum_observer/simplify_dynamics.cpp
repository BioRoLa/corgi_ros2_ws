...
namespace simplify_dynamics
{
    ...
    {
        // The symbolic core below assumes at least 12 states are present.
        if (q.size() < 12 || q_dot.size() < 12 || I_c.size() < 4)
        {
            M.setZero();
            C.setZero();
            G.setZero();
        ...
        // ABAD extension: appended gamma states are currently modeled as decoupled
        // channels with diagonal inertia and zero C/G coupling.
        int ext_dim = M.rows();
        ext_dim = std::min(ext_dim, static_cast<int>(M.cols()));
        ext_dim = std::min(ext_dim, static_cast<int>(C.rows()));
        ext_dim = std::min(ext_dim, static_cast<int>(C.cols()));
        ext_dim = std::min(ext_dim, static_cast<int>(G.size()));
        ext_dim = std::min(ext_dim, static_cast<int>(D.rows()));
        ext_dim = std::min(ext_dim, static_cast<int>(D.cols()));
        for (int i = 12; i < ext_dim; ++i)
        {
            M(i, i) = 0.05;
        }
    ...
    void compute_mass_matrix(
        ...
        Eigen::Ref<Eigen::MatrixXd> M)
    {
        Eigen::MatrixXd C_dummy = Eigen::MatrixXd::Zero(M.rows(), M.cols());
        Eigen::VectorXd G_dummy = Eigen::VectorXd::Zero(M.rows());
        Eigen::MatrixXd D_dummy = Eigen::MatrixXd::Zero(M.rows(), M.cols());
        Eigen::VectorXd q_dot_dummy = Eigen::VectorXd::Zero(std::max<int>(12, q.size()));
