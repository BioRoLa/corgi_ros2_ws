...
// Eigen::Vector2d LegModel::pointOnRimByGamma(
//     double theta,
//     double beta,
//     int rim,
//     double gamma
// ) {
//     // 1) 更新到當前姿態（forward + rotate）
//     forward(theta, beta, /*vector=*/false);

//     // 2) 取出複數表示的 圓心 O 與弧段起點向量 V0 = P0 - O
//     std::complex<double> O, V0;
//     switch (rim) {
//       case 1: O = U_l_c; V0 = H_l_c - U_l_c; break;  // left upper
//       case 2: O = L_l_c; V0 = F_l_c - L_l_c; break;  // left lower
//       case 3: O = L_l_c; V0 = G_c   - L_l_c; break;  // G segment
//       case 4: O = L_r_c; V0 = F_r_c - L_r_c; break;  // right lower
//       case 5: O = U_r_c; V0 = H_r_c - U_r_c; break;  // right upper
//       default:
//         throw std::runtime_error("pointOnRimByGamma: invalid rim id");
//     }

//     // 3) 根據 gamma 範圍選轉角
//     const double deg50 = 50.0 * M_PI / 180.0;
//     std::complex<double> rotated;
//     if (gamma >= -M_PI   && gamma < -deg50) {
//         rotated = std::polar(1.0, gamma + M_PI) * V0;
//     }
//     else if (gamma >= -deg50 && gamma < 0.0) {
//         rotated = std::polar(1.0, gamma + deg50) * V0;
//     }
//     else if (gamma >= 0.0 && gamma <  deg50) {
//         rotated = std::polar(1.0, gamma) * V0;
//     }
//     else if (gamma >= deg50 && gamma <  M_PI) {
//         rotated = std::polar(1.0, gamma - deg50) * V0;
//     }
//     else {
//         // 超出輪緣範圍 => Eq.(2.28) 腳尖處理
//         std::complex<double> VG = G_c - L_l_c;
//         std::complex<double> tip = L_l_c + (r / R) * std::polar(1.0, gamma) * VG;
//         return Eigen::Vector2d(tip.real(), tip.imag());
//     }

//     // 4) 一般輪緣點 => Eq.(2.27)
//     std::complex<double> P = O + (r / R) * rotated;
//     return Eigen::Vector2d(P.real(), P.imag());
// }