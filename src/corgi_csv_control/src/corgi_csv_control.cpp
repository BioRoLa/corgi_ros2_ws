...
int main(int argc, char **argv) {
    ...
    if (softening_enabled) {
        ...
        if (softener.load(phase_path, td_lead_ms, td_hold_ms, td_ramp_ms, phase_error)) {
            RCLCPP_INFO(
                ...
                phase_path.c_str(), softener.rows(), softener.touchdown_count(),
                soft.kp_scale, soft.kp_h_scale, soft.kd_scale,
                td_lead_ms, td_hold_ms, td_ramp_ms);
        } else {
            RCLCPP_INFO(
                node->get_logger(),
                "Touchdown softening OFF (%s: %s); using constant kp=%.0f ki=%.0f kd=%.2f",
                phase_path.c_str(), phase_error.c_str(), kBaseKp, kBaseKi, kBaseKd);
        }
    } else {
        RCLCPP_INFO(
            node->get_logger(),
            "Touchdown softening disabled by parameter; using constant kp=%.0f ki=%.0f kd=%.2f",
            kBaseKp, kBaseKi, kBaseKd);
    }
    ...
    for (int i=0; i<kTransformRows; i++){
        ...
        const std::array<float, kLegCount> *blend_ptr = softener.active() ? &blend : nullptr;
        for (size_t module = 0; module < motor_cmds.size(); ++module){
            ...
            apply_gains(cmd, module, blend_ptr, soft);
        }
