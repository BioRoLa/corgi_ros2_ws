...
Legged::Legged(ros::NodeHandle& nh) : GaitSelector(nh)
{
    
}
// not initial duty / with initial duty
void Legged::Initial() 
{
    // std::cout<< "initial legged" << std::endl;
    // read current eta 
    double init_theta[4] = {eta[0][0], eta[1][0], eta[2][0], eta[3][0]};
    // double init_beta[4]  = {-init_eta[1], init_eta[3], init_eta[5], -init_eta[7]};
    double init_beta[4]  = {-eta[0][1], -eta[1][1], -eta[2][1], -eta[3][1]};
    // std::cout<< "receive initial" << std::endl;
    // std::cout<< "init_theta: " << init_theta[0]*180/PI << ", " << init_theta[1]*180/PI << ", " << init_theta[2]*180/PI << ", " << init_theta[3]*180/PI << std::endl;
    // std::cout<< "init_beta: " << init_beta[0] *180/PI<< ", " << init_beta[1]*180/PI << ", " << init_beta[2] *180/PI<< ", " << init_beta[3]*180/PI << std::endl;
    // Get foothold in hip coordinate from initial configuration
    relative_foothold[4][2] = {0.0};
    int current_rim = 0;
    // std::cout<< "0" << std::endl;
    for (int i=0; i<4; i++) {
...
void Legged::next_Step() {
    ...
    for (int i=0; i<4; i++) {
        ...
        if ((duty[i] > (1 - swing_time)) && swing_phase[i] == 0) {
            ...
            for (int j = 0; j < 5; j++) {   // G, L_l, U_l
                ...
                std::array<double, 2> contact_point = {
                    foothold[i][0] - (next_hip[i][0] + swing_hip_move_d),
                    -stand_height + contact_height
                };
            
                // Convert std::array to a C-style array pointer using .data()
                result_eta = leg_model.inverse(contact_point.data(), touch_rim_list[j]);
        ...
        if (swing_phase[i] == 0) { // Stance phase
            ...
            result_eta = leg_model.inverse(curve_point.data(), "G");
        }//end if else
...
// void Legged::set_eta(std::array<std::array<double, 4>, 2> eta_) {
//     this->theta = eta_[0];
//     this->beta  = eta_[1];
// }//end set_eta

// void Legged::set_duty(std::array<double, 4> duty_) {
//     for (int i=0; i<4; i++) {
...
// std::array<int, 4> Legged::get_step_count() {
//     return this->step_count;
// }//end get_step_count

// std::array<int, 4> Legged::get_swing_phase() {
//     return this->swing_phase;
// }//end get_step_count

// std::array<double, 4> Legged::get_duty() {
//     return this->duty;
// }//end get_duty

// bool Legged::if_touchdown() {
