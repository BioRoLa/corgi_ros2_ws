...
static const double vel            = 0.1;      // 速度 (m/s)
...
// CONSTANTS & LIMITS
// ------------------------------------------------------------
constexpr double kDeg  = M_PI / 180.0;             // deg → rad
constexpr double kThetaMin     = 17.0 * kDeg;      // θ lower bound (rad)
double kTheta     = 17.0 * kDeg;      // θ lower bound (rad)
constexpr double kThetaMax     = 160.0 * kDeg;     // θ upper bound (rad)
constexpr double kThetaVelLim  = 40 * kDeg;       // |θ̇|max (rad/s)
constexpr double kSearchStep   = 1.0 * kDeg;       // θ_via increment (rad)
// ------------------------------------------------------------
// Cubic Hermite (scalar)
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// 產生兩段 Hermite 並填入 DataPoint (θ 先降後升)
// ------------------------------------------------------------
static void generateSegmentedHermite(LegModel &leg,
                                     ...
                                     std::vector<DataPoint> &out)
{
    ...
    for (unsigned i = 0; i <= N; ++i) {
        ...
        Eigen::Vector2d p = pointOnRimByAlpha(leg, theta, beta, rim, alpha);
        out.push_back({theta, beta, p.x(), p.y(), "swing"});
    }
}
bool maxThetaVelocity(const std::vector<DataPoint> &data)
{
    
    for (size_t i=1; i<data.size(); ++i) {
        double d_theta = data[i].theta - data[i-1].theta;
        double vel = std::fabs(d_theta *1000);
        
        if (vel>= kThetaVelLim){
            // std::cout << "d_theta = " << d_theta << ", vel = " << vel << std::endl;
            return false;
        }
    }
    return true;
...
// ------------------------------------------------------------
// PUBLIC API :  planSwing()
// ------------------------------------------------------------
void planSwing(LegModel &leg,
               double theta_start, double beta_start,
               double theta_end,   double beta_end,
               unsigned N, int rim, double alpha,
               std::vector<DataPoint> &data_out)
{
    Eigen::Vector2d q_via;
    q_via(1) = 0.5 * (beta_start + beta_end);   // β midpoint

    for (q_via(0) = kTheta; q_via(0) <= std::max(theta_start,theta_end); q_via(0) += kSearchStep) {
        generateSegmentedHermite(leg, theta_start, beta_start,
                                 theta_end,   beta_end,
                                 q_via, N, rim, alpha, data_out);
        if (maxThetaVelocity(data_out)) {
            std::cout << "Found feasible trajectory with q_via(0) = " << q_via(0)/kDeg<< std::endl;
            return;   // first feasible trajectory found
        }
        else{
            std::cout << "q_via(0) = " << q_via(0)/kDeg << " exceeds max θ velocity, trying next..." << std::endl;
        }
    }
}
...
{   
    ...
    // );
    planSwing(leg,eta_current[0], eta_current[1], eta_temp_swing[0], eta_temp_swing[1],
              N, rim_id, alpha0,
              data_point);
...
// ———— 主程式：手動巢狀搜尋 ————
int main() {
    std::ofstream file2("opt_10.csv");
    file2 << "theta,beta,x,y,phase\n";
    std::vector<DataPoint> data;
    Terrain terrain(-0.149);
    // terrain.addPlain(0.08);
    terrain.addSlope((2.23716094422), 0.04080820618);
    terrain.addSlope((2.23716094422), 0.04080820618);
    terrain.addSlope((2.23716094422), 0.04080820618);
    terrain.addSlope((2.23716094422), 0.04080820618);
    terrain.addPlain(5);
