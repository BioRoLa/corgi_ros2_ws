...
std::shared_ptr<rclcpp::Rate> rate_ptr;
// tools
                    ...
                    double hip_x, double stand_h, double hip_z)
{
    ...
    // A: L_l_c → [F_l_c, G_c]
    {
        ...
        if (sampleSector(C + hipOff, leg.F_l_c + hipOff, leg.G_c + hipOff, R))
            return true;
    }
    // B: L_r_c → [G_c, F_r_c]
    {
        ...
        if (sampleSector(C + hipOff, leg.G_c + hipOff, leg.F_r_c + hipOff, R))
            return true;
    }
    // C: U_r_c → [H_r_c, F_r_c]
    {
        ...
        if (sampleSector(C + hipOff, leg.H_r_c + hipOff, leg.F_r_c + hipOff, R))
            return true;
    }
    // D: U_l_c → [H_l_c, F_l_c]
    {
        ...
        if (sampleSector(C + hipOff, leg.H_l_c + hipOff, leg.F_l_c + hipOff, R))
            return true;
    }
    ...
    // G-sector: G_c → [extend toward L_l_c, extend toward L_r_c]
    {
        ...
        if (sampleSector(C + hipOff, P0 + hipOff, P1 + hipOff, leg.r))
            return true;
    }
    ...
    if (n == 4)
    {
        if (duty == 0.8)
        {
            ...
            if (psi_min > 0.087) // 5°(≈0.087 rad) 以上視為安全
                RCLCPP_INFO_STREAM(g_node->get_logger(), "Stable in 4 leg! score = " << psi_min << " rad");
            else
                RCLCPP_WARN_STREAM(g_node->get_logger(), "Unstable in 4 leg ! score = " << psi_min << " rad");
            // 2. 確認離地後勝三隻腳也是穩的
            support.erase(
                               ...
                               int swing_idx, double duty)
{
    ...
    if (psi_min >= 0)
    { // 5°(≈0.087 rad) 以上視為安全
        RCLCPP_INFO_STREAM(g_node->get_logger(), "Stable!  score = " << psi_min << "  Nm");
        stability = true;
    }
    else
    {
        RCLCPP_WARN_STREAM(g_node->get_logger(), "Unstable! score = " << psi_min << "  Nm");
    }
    return psi_min;
}
...
void Hybrid_Step(Robot &robot, LegModel &leg_model, int leg_index)
{
    ...
    if (robot.legs[leg_index].duty < 0)
...
int main(int argc, char **argv)
{
    ...
    std::thread spin_thread([&exec]()
                            { exec.spin(); });
    // motor state sub
    motor_state_sub_ = g_node->create_subscription<corgi_msgs::msg::MotorStateStamped>(
