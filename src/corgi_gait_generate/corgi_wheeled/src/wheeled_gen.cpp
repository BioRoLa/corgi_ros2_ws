...
Wheeled::Wheeled(rclcpp::Node::SharedPtr node, const std::string &control_mode)
    : GaitSelector(node), WheeledCmd(node, control_mode)
{
    beta_adjustment = 0.0;
    beta_adjustment_l = 0.0;
    beta_adjustment_r = 0.0;

    wheel_cmd_sub_ = node_->create_subscription<corgi_msgs::msg::WheelCmd>(
        "/wheel_cmd", 10, std::bind(&Wheeled::wheelCmdCallback, this, std::placeholders::_1));
    steer_cmd_sub_ = node_->create_subscription<corgi_msgs::msg::SteeringCmdStamped>(
        "/steer/command", 10, std::bind(&Wheeled::steerCmdCallback, this, std::placeholders::_1));
}
...
void Wheeled::wheelCmdCallback(const corgi_msgs::msg::WheelCmd::SharedPtr msg)
{
    ...
    motor_cmd.header.stamp = node_->now();
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        {
            motor_cmd_modules[i]->beta = (i == 1 || i == 2) ? (motor_state_modules[i]->beta - beta_adjustment_r) : (motor_state_modules[i]->beta + beta_adjustment_l);
        }
        else
        {
            // Rotate in place
...
void Wheeled::steerCmdCallback(const corgi_msgs::msg::SteeringCmdStamped::SharedPtr msg)
{
