    ...
    while (ros::ok()) {
        ...
        switch (state) {
            ...
            case WALK:
                ...
                max_step_length_last = (-D/2.0 - 0.05 - 0.15 - hip_x)*2; // step length if from current pos to min_keep_stair_d, step_length*(swing_phase + (1-swing_phase)/2) = foothold_x - hip_x
