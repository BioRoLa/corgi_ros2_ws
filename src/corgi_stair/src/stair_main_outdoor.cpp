    ...
    while (rclcpp::ok())
    {
        ...
        case WALK:
            ...
            if (tfBuffer.canTransform("map", "zedxm_camera_center", tf2::TimePointZero, tf2::durationFromSec(0.0)))
            {
                ...
                {
                    RCLCPP_WARN_THROTTLE(node->get_logger(), *node->get_clock(), 1000, "TF lookup failed even after canTransform: %s", ex.what());
                }
            }
            else
            {
                RCLCPP_WARN_THROTTLE(node->get_logger(), *node->get_clock(), 1000, "TF not available at this moment");
            }
            ...
            if (change_step_length && walk_gait.if_touchdown() && (swing_phase[0] == 1 || swing_phase[1] == 1))
            { // walk_gait apply new step_length
                std::cout << "step_length:" << to_enter_d / (min_steps / 2.0) << std::endl;
            }
            CoM[0] += velocity / sampling_rate; // expected CoM x position
            command_pitch_CoM << node->now().seconds() << "," << (int)trigger_msg.enable << "," << 0.0 << "," << CoM[0] << "," << stand_height << "\n";
            command_count++;
            break;
        case STAIR:
            ...
            if (stair_climb.any_no_stair())
            {
                if ((stair_count == 0 || plane_msg.vertical.size() >= stair_count + 1) && plane_msg.horizontal.size() >= stair_count + 2)
                {
                    ...
                    std::cout << "Stair H: " << H << " m, D: " << D << " m." << std::endl;
                    stair_size_csv << node->now().seconds() << "," << (int)trigger_msg.enable << "," << D << "," << H << "\n";
                    stair_count++;
                } // end if
            } // end if
            ...
            command_pitch_CoM << node->now().seconds() << "," << (int)trigger_msg.enable << "," << pitch << "," << CoM[0] << "," << CoM[1] << "\n";
            command_count++;
            break;
        case UPPER_WALK:
            if (last_state != state)
        ...
        case WALK:
            ...
            if (walk_gait.if_touchdown() && (swing_phase[0] == 1 || swing_phase[1] == 1))
            { // hind leg touched down (front leg start to swing)
                if (hip_x + (max_step_length / 2) >= to_stair_d - optimal_foothold)
                { // max next foothold >= keep_stair_d_front_max, to swing up stair
                    state = STAIR;
        ...
        case UPPER_WALK:
            ...
            if (step_count[0] >= 2 && step_count[1] >= 2 && step_count[2] >= 2 && step_count[3] >= 2)
